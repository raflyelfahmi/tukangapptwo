  ...
  void onInit() {
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) async {
      if (user != null) {
        ...
        print('User role: $role'); // Tambahkan log untuk role user
        if (role == 'pemesan') {
          fetchUsers(user.uid);
        } else {
      ...
      if (snapshot.value != null) {
        ...
        if (values is Map<dynamic, dynamic>) {
          values.forEach((key, value) {
            ...
            if (value is Map<String, dynamic> && value['role'] == 'tukang' && value['rolesKeunggulan'] != null && (value['rolesKeunggulan'] as List).contains('Painter atau Mengecat')) {
            ...
            } else if (value is Map && value['role'] == 'tukang' && value['rolesKeunggulan'] != null && (value['rolesKeunggulan'] as List).contains('Painter atau Mengecat')) {
              if (key != currentUserUid) {
                userList.add({
                  'uid': key,
                  'name': value['name'],
...
class PainterView extends StatelessWidget {
  final PainterController painterController = Get.put(PainterController());
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(() {
        if (painterController.accessDenied.value) {
          return Center(
            child: Text(
              'Akses Ditolak',
        ...
        } else if (painterController.isLoading.value) {
          return const Center(child: CircularProgressIndicator());
        } else if (painterController.users.isEmpty) {
          return const Center(
            child: Text(
              'No users found',
            ...
            child: Column(
              ...
              children: <Widget>[
                ListView.builder(
                  ...
                  itemCount: painterController.users.length,
                  itemBuilder: (context, index) {
                    var user = painterController.users[index];
                    return InkWell(
                      onTap: () {
